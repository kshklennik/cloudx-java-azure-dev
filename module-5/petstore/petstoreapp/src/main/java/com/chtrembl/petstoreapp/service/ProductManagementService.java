...
public class ProductManagementService {
    ...
    public Collection<Product> getProductsByCategory(String category, List<Tag> tags) {
        ...
        try {
            ...
            this.sessionUser.getTelemetryClient().trackEvent(
                    String.format("Custom event ProductService. User: %s. Session: %s",
                            this.sessionUser.getName(), sessionUser.getSessionId()),
                    this.sessionUser.getCustomEventProperties(), null);
            ...
            log.info("Returning {} items to the user", products.size());

            this.sessionUser.getTelemetryClient().trackMetric("ProductsFound_" + category, products.size());
